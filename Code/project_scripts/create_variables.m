...
input_dir = [root_dir filesep 'Temp' filesep 'Create variables' ...
    filesep 'Input'];
output_dir = [root_dir filesep 'Temp' filesep 'Create variables' ...
    filesep 'Output'];
next_dir = [root_dir filesep 'Temp' filesep 'Create ERP measures' ...
    filesep 'Input'];
...
% will be filled in with content to be exported to Excel
export_to_excel = {}; 
...
ff_portfolios_clean = retime(ff_portfolios_ts, 'monthly', ...
    'fillwithmissing');
mom_portfolios_clean = retime(mom_portfolios_ts, 'monthly', ...
    'fillwithmissing');
...
eps_1yr_ahead = thomson_monthly.EPS_y1 .* (1-alpha) + ...
    thomson_monthly.EPS_y2 .* alpha;
eps_2yr_ahead = thomson_monthly.EPS_y2 .* (1-alpha) + ...
    thomson_monthly.EPS_y3 .* alpha;
eps_3yr_ahead = thomson_monthly.EPS_y3 .* (1-alpha) + ...
    thomson_monthly.EPS_y4 .* alpha;
eps_4yr_ahead = thomson_monthly.EPS_y4 .* (1-alpha) + ...
    thomson_monthly.EPS_y5 .* alpha;

thomson_clean = array2table([datenum(thomson_monthly.date), ... 
    eps_1yr_ahead, eps_2yr_ahead, eps_3yr_ahead, eps_4yr_ahead]);
thomson_clean.Properties.VariableNames = {'date', 'eps_1yr_ahead', ...
    'eps_2yr_ahead','eps_3yr_ahead','eps_4yr_ahead'}; 
thomson_clean.date = datetime(thomson_clean.date, ...
    'ConvertFrom', 'datenum');
...
for j = 1:size(temp_data,2)
    ...
    first_day = [first_day; max(datenum(1900,1,1), ...                      % first preiod
        datenum(thomson_clean.date(first_nonnan)))]; 
    last_day = [last_day; datenum(thomson_clean.date(last_nonnan))];        % last period
end
export_to_excel = [export_to_excel; ...
    [{'eps_1yr_ahead';'eps_2yr_ahead';'eps_3yr_ahead';'eps_4yr_ahead'}, ...
    repmat({'ThomsonReuters I/B/E/S'},4,1), repmat({'monthly'},4,1), ...
    first_day,last_day,desc]];
...
% combine all time series into one 
data_ts = synchronize(damodaran_clean, cfo_clean, ff_clean, mom_clean, ...
    ff_portfolios_clean, mom_portfolios_clean, yields_clean, ...
    tips_clear, nyfed_clear, shiller_clean, thomson_clean, fred_clean, ...
    compustat_clean, bw_clean, cay_clean);
...
export_to_excel = [export_to_excel;[{'dp'},{'Shiller'},{'monthly'}, ...
    first_day,last_day,desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'ep'},{'Shiller'},{'monthly'}, ...
    first_day,last_day,desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'inflation'},{'Shiller'}, ...
    {'monthly'},first_day,last_day,desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan)); % last period
export_to_excel = [export_to_excel;[{'term_spread'}, {'Fed Board'}, ...
    {'monthly'}, first_day, last_day,desc]];

% net issuance
share_equity_issuance = 100 * data_ts.equity_issuance ./ ...
    (data_ts.equity_issuance+data_ts.debt_issuance);
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan)); % last period
export_to_excel = [export_to_excel;[{'share_equity_issuance'}, ...
    {'Jeffrey Wurgler'}, {'monthly'}, first_day,last_day,desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'book_per_share'},{'Compustat'}, ...
    {'monthly'},first_day,last_day,desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'lag_book_per_share'}, ...
    {'Compustat'},{'monthly'},first_day,last_day,desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'book_equity'},{'Compustat'}, ...
    {'monthly'},first_day,last_day,desc]];
...
first_day = max((datenum(1900,1,1)),datenum(data_ts.Time(first_nonnan)));   % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'real_book_equity'}, ...
    {'Compustat and Shiller'},{'monthly'},first_day,last_day,desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'bm'}, {'Compustat'}, {'monthly'}, ...
    first_day,last_day,desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'smooth_EP'}, {'Shiller'}, ...
    {'monthly'}, first_day, last_day,desc]];

% return on equity
roe = data_ts.E ./ ((1+data_ts.inflation/(100*12)) .* ...
    (data_ts.real_book_equity));
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'roe'}, {'Compustat and Shiller'}, ...
    {'monthly'}, first_day, last_day,desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'real_rf_mean'}, ...
    {'Fama-French and Shiller'}, {'monthly'}, first_day, last_day, desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'payout_mean'}, {'Shiller'}, ...
    {'monthly'}, first_day, last_day, desc]];

% historical average of real earnings growth
real_earnings = data_ts.E./data_ts.CPI;
earnings_growth = 100 * 12 * (real_earnings(2:end) ./ ...
    real_earnings(1:end-1) - 1);
nan_temp = isnan(earnings_growth);
earnings_growth = cumsum(earnings_growth, 'omitnan') ./ ...
    (1:length(earnings_growth))';
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'real_earnings_gr_mean'}, ...
    {'Shiller'}, {'monthly'}, first_day, last_day, desc]];

% growth-adjusted dividend-price ratio
data_ts.dp_growth_adj = data_ts.dp + (1-data_ts.payout_mean/100) .* ...
    data_ts.roe;
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'dp_growth_adj'}, ...
    {'Compustat and Shiller'}, {'monthly'}, first_day, last_day, desc]];

% growth-adjusted earnings-price ratio
data_ts.ep_growth_adj = (data_ts.payout_mean/100) .* data_ts.ep + ...
    (1-data_ts.payout_mean/100) .* data_ts.roe;
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day = datenum(data_ts.Time(last_nonnan));                              % last period
export_to_excel = [export_to_excel;[{'ep_growth_adj'}, ...
    {'Compustat and Shiller'}, {'monthly'}, first_day, last_day, desc]];

% growth-adjusted smooth earnings-price ratio
data_ts.smooth_ep_growth_adj = (data_ts.payout_mean/100) .* ...
    data_ts.smooth_EP+(1-data_ts.payout_mean/100) .* data_ts.roe;
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'smooth_ep_growth_adj'}, ...
    {'Compustat and Shiller'}, {'monthly'}, first_day, last_day, desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'bm_growth_adj'}, ...
    {'Compustat and Shiller'}, {'monthly'}, first_day, last_day, desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'dp_growth_adj_rf'}, ...
    {'Compustat, Shiller and Fama-French'}, {'monthly'}, first_day, ...
    last_day, desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'ep_growth_adj_rf'}, ...
    {'Compustat, Shiller and Fama-French'}, {'monthly'}, first_day, ...
    last_day, desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'smooth_ep_growth_adj_rf'}, ...
    {'Compustat, Shiller and Fama-French'}, {'monthly'}, first_day, ...
    last_day, desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'bm_growth_adj_rf'}, ...
    {'Compustat, Shiller and Fama-French'}, {'monthly'}, first_day, ...
    last_day, desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'real_D_growth'}, {'Shiller'}, ...
    {'monthly'}, first_day, last_day, desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'real_E_growth'}, {'Shiller'}, ...
    {'monthly'}, first_day, last_day, desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'real_P_growth'}, {'Shiller'}, ...
    {'monthly'}, first_day, last_day, desc]];
...
first_day = max(datenum(1900,1,1),datenum(data_ts.Time(first_nonnan)));     % first preiod
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'dividend_yield'}, {'Shiller'}, ...
    {'monthly'}, first_day, last_day, desc]];

% realized equity risk premium
data_ts.realized_erp = 100 * 12 * ((data_ts.P+data_ts.D/12)./ ...
    lagged.P-1) - data_ts.RF;
...
last_day =  datenum(data_ts.Time(last_nonnan));                             % last period
export_to_excel = [export_to_excel;[{'realized_erp'}, {'Shiller'}, ...
    {'monthly'}, first_day, last_day, desc]];
...
export_to_excel.Properties.VariableNames = {'Names', 'Source', ...
    'Frequency', 'FirstPeriod', 'LastPeriod', 'Description'};               % headers of excel file
