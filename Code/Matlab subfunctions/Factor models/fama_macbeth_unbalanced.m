...
%   LAMBDA = FAMA_MACBETH(RETURNS,BETAS) gives a 1-by-K vector of factor
%   risk premia. RETURNS is a T-by-1 cell, each of which contains an N_t-by-1
%   matrix of observations of the excess returns of N traded assets at time t. 
%   Note that the number observations for different time periods t can be
%   different (i.e. the panel can be unbalanced). BETAS is a T-by-1, where each cell
%   contains an N_t-by-K matrix of factor exposures. Missing observations
%   should be indicated by NaN's.
%
...
%   all pricing errors are jointly zero (stats.pAllErrors); the t-statistic
%   for lambda_t at each time t (stats.tLambda_t); the standard deviation
%   of lambda_t at each time t (stats.stdLambda_t; the R-squared of each cross-sectional
%   regression (stats.R_t).
%
...
if ~strcmpi(method,'Constant betas')
    ...
    for t=1:T
        ...
        missing = any(isnan(beta_t),2) | isnan(returns(t,:))';
        beta_t(missing,:) = [];use_returns = returns(t,~missing)';
        
        N = size(beta_t,1);
        if constant == 0
            if N>1
                % find weighting matrix
                wlsCoeff = regress(use_returns,beta_t);
                wlsError = use_returns-beta_t*wlsCoeff;
                ...
                weightedReturns = sqrtGls\use_returns;
                % run one cross-sectional regression for each time period
                % (without a constant)
                lambda_t(t,:) = regress(weightedReturns,beta_t);
                errors_t(t,~missing) = sqrtGls*(weightedReturns-beta_t*lambda_t(t,:)');
                xs_std = regstats(weightedReturns,beta_t,eye(size(beta_t,2)),{'covb','rsquare','tstat'}); % get cross-sectional standard error
                tLambda_t(t,:) = xs_std.tstat.t;
                stdLambda_t(t,:) = diag(xs_std.covb);
                R_t(t,:) = xs_std.rsquare;
            end
        elseif constant == 1
            % run one cross-sectional regression for each time period
            % (with a constant)
            if N > 1
                % find weighting matrix
                wlsCoeff = regress(use_returns,[vecOne beta_t]);
                wlsError = use_returns-[vecOne beta_t]*wlsCoeff;
                if strcmpi(var_method,'ols')
                    gls = eye(N);
                elseif strcmpi(var_method,'wls')
                    gls = T*diag(diag(wlsError*wlsError'));
                ...
                errors_t(t,~missing) =  sqrtGls*(weightedReturns-beta_t*coeff_t(t,:)');
                
                xs_std = regstats(weightedReturns,beta_t,{'covb','rsquare','tstat'}); % get cross-sectional standard error
                tLambda_t(t,:) = xs_std.tstat.t;
                stdLambda_t(t,:) = diag(xs_std.covb);
                R_t(t,:) = xs_std.rsquare;
            end
    ...
    lambda_t(isnan(lambda_t))=nan;
    T = size(lambda_t,1);
    lambda = nanmean(lambda_t,1);
    errors = nanmean(errors_t,1);
    ...
    if strcmpi(method,'Standard')
        ...
        stats.varErrors = nancovall(errors_t,1)/T;
        
        % compute Chi^2 statistic and pvalue for the test that all errors are jointly
        % zero
        if any(any(isnan(stats.varErrors)))
            stats.varErrors(isnan(stats.varErrors)) = 0;% if some correlations can't be computed due to incomplete data, assume they are zero (may not give a positive definite matrix)
            allErrorsChi2 = T*errors*(stats.varErrors\errors'); % Chi^2 with N degrees of freedom
            
            if allErrorsChi2 < 0 % then stats.varErrors is not positive definite
                stats.varErrors = diag(diag(stats.varErrors)); % assume all cross-correlations are zero
            end
        end
        allErrorsChi2 = T*errors*(stats.varErrors\errors'); % Chi^2 with N degrees of freedom
...
elseif strcmpi(method,'Constant betas')
    ...
    meanRet = (nanmean(returns,1)');
    meanBetas = cell2mat(cellfun(@nanmean,betas,'UniformOutput',false)');
...
% fill in cross-sectional statistics
stats.tLambda_t = tLambda_t;
stats.stdLambda_t = stdLambda_t;
stats.R_t= R_t;
